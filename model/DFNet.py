...
class FrameSynthesis(nn.Module):
    def __init__(self, fusion_Feat, RDB_Num, Growth_rate, Conv_Num):
        ...
        self.DFNet = DFNet(fusion_Feat, RDB_Num, Growth_rate, Conv_Num)
        self.Filter = DynFilter()

    def forward(self, in_tensor):
        DF = F.softmax(self.DFNet(in_tensor), dim=1)

        candidates = in_tensor[:, 3:-3, :, :]

        R = self.Filter(candidates[:, 0::67, :, :], DF)
        G = self.Filter(candidates[:, 1::67, :, :], DF)
        B = self.Filter(candidates[:, 2::67, :, :], DF)
